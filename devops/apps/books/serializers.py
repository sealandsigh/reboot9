...
class BookSerializer(serializers.ModelSerializer):
    # publisher = PublisheSerializer() # 一对多，默认显示PublishSerializer定义的所有列
    # authors = AuthorSerializer(many=True) # 多对多，默认显示AuthorSerializer所有的列
    # publication_date = serializer.DateTimeField(format="%Y-%m-%d") # 后端格式化序列化输出时间
    ...
    def to_representation(self, instance):
        ...
        # 多对多，相当于多对对的正向查询，获取当前书的作者，SQL: Book.objects.get(pk=1).authors.all()
        ...
        # 将关联表需要序列化输出的列处理为json，也加入序列化大字典中，这样就能序列化当前表和关联表所有想展示的字段了
        ret["publisher"] = {
            "id": publisher_obj.id,
    ...
    # 重写create方法，源码中已经对单表，一对多，多对多关系做了处理，此次为了学习调试方便重写
    def create(self, validated_data):
    ...
    def update(self, instance, validated_data):
        print("aaa {}".format(validated_data))
        print("bbb {}".format(instance))
        author_list = validated_data.pop('authors', [])
        self.Meta.model.objects.filter(id=instance.id).update(**validated_data)
        # 多对多添加的两种写法
